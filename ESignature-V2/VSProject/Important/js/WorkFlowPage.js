...
function PositionControls(ControlsToPlace) {
	Array = [];
	//debugger;
	AdminControlsPlacedList = ControlsToPlace;
		...
		for (var i = 0; i < length-1; i++) {
			for (var j = 3; j < finalString.split(':')[i].split(',').length;) {
				...
				if (SignTag == 'Name') {
					...
					$("#pageContainer").append('<div id=' + ID + '  data-id="-1" data-page="0" data-toggle="X" data-valore="X" data-x=' + x + ' data-y=' + y + ' style = "transform: translate(' + x + 'px,' + y + 'px);display: block; position: absolute; left: 0px; top: 0px; " clonable="false"><span class=""></span><span class=""><label>Sam Joseph</label></span></div>');
				}
				//For Date Tag
				/*if (SignTag == 'Date') {
					var ID = "Date_" + i;
